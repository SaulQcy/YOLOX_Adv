...
import utils
from adversarial_attack import postprocess


class SimBA:
    
    def __init__(self, obj, dataset, image_size):
    ...
    def normalize(self, x):
        from adversarial_attack.SimBA.utils import apply_normalization
        return apply_normalization(x, self.dataset)

    def get_probs(self, x, y):
        output = self.model(self.normalize(x.cuda())).to(x.device)
        output = postprocess(
            output, self.obj.num_classes, self.obj.confthre,
            self.obj.nmsthre, class_agnostic=True
        )[0]

        if output is None or y is None or len(y) == 0:
            return torch.tensor(0.0, device=x.device)

        l = min(output.shape[0], y.shape[0])
        scores = []
        for i in range(l):
            pred_vec = output[i]
            gt_vec = y[i].to(x.device)

            # L1 距离归一化为 [0, 1] 范围相似度，维度一致性假设 pred_vec 和 gt_vec 相同
            l1_dist = torch.nn.functional.l1_loss(pred_vec, gt_vec, reduction='mean')
            score = 1 - l1_dist  # 越相似分数越高
            scores.append(score)
    ...
    def simba_single(self, x, y, num_iters=10000, epsilon=0.2, targeted=False):
        ...
        with torch.no_grad():
            y = self.model(x.cuda())
        y = postprocess(
            y, self.obj.num_classes, self.obj.confthre,
            self.obj.nmsthre, class_agnostic=True
        )[0]
        ...
        for i in tqdm(range(num_iters)):
            diff = torch.zeros(n_dims).to(x)
            diff[perm[i]] = epsilon
            left_prob = self.get_probs((x - diff.view(x.size())).clamp(-1, 1), y)
            if targeted != (left_prob < last_prob):
                x = (x - diff.view(x.size())).clamp(-1, 1)
                last_prob = left_prob
            else:
                right_prob = self.get_probs((x + diff.view(x.size())).clamp(-1, 1), y)
